...
const Viewuser = ({users,userid,setviewusermodal,viewusermodal}) => {
    const [user,setuser]=useState()
    useEffect(()=>{
        const find=users?.filter(ele=>ele._id==userid)
        find&&setuser(find[0])
     
    },[userid,users])
  return (
    viewusermodal&&
    <div className='viewusermodal chatviewusermodal'>
        <span className='badge text-primary'>User Details</span>
